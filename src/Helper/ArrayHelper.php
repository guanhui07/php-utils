...
class ArrayHelper
{
    ...
    public static function get($array, $key = null, $default = null)
    {
        ...
        foreach (explode('.', $key) as $segment) {
            if (static::accessible($array) && static::exists($array, $segment)) {
                ...
                return CallbackHelper::value($default);
            }
        }
    ...
    {
        return is_array($value) || $value instanceof ArrayableInterface;
    }
    ...
    public static function flatten(array $array, int $depth = PHP_INT_MAX): array
    {
        ...
        foreach ($array as $item) {
            $item = $item instanceof CollectionHelper ? $item->all() : $item;
    ...
    {
        if (is_null($callback)) {
            return empty($array) ? CallbackHelper::value($default) : end($array);
        }
    ...
    public static function collapse($array)
    {
        ...
        foreach ($array as $values) {
            if ($values instanceof CollectionHelper) {
                $values = $values->all();
     ...
     * 数组数据查找字段匹配返回相同值
     *
     *      例子： $array = [['id' => 1, 'name' => 'kaka'], ['id' => 2, 'name' => 'tyl'], ['id' => 3, 'name' => 'tyl']];
     *            想要获取 $array 数组中 name 字段值都为 tyl 的数据集
     *            调用: array_field_match($array, 'name', 'tyl')
     *      ⚠ 调用注意点:  1.$array 务必是二维数组且必须是 key=>value 键值对,即不能是 [['kaka'], ['raylin666'], ['swoolefly']]
     *                    2.该方法是采用全等匹对，请确保参数数据类型和数组中字段数据类型相一致。
     *
     * @param array $array  必须是二维数组且必须是 key=>value 键值对,即不能是 [['kaka'], ['raylin666'], ['swoolefly']]
     * @param       $field  需要匹配的二维数组中字段名称
     * @param       $search 需要匹配的值
     * @return array
     */
    public static function fieldMatch(array $array, $field, $search) : array
    {
        if (!$field || !$search) {
            return [];
        }

        return array_filter($array, function ($res) use ($field, $search) {
            if (isset($res[$field])) {
                return $res[$field] === $search;
            }
     ...
     * 获取指定数组键名的键值对
     *
     *      例子： $array = ['d' => ['kaak'], 'xe1' => ['xd'], 'wf2' =>['shuag']];
     *            想要获取 $array 数组中 xe1和wf2 字段数据集
     *            调用: array_intersect_key_flip($array, ['xe1', 'wf2'])
     *
     * @param array $array  数据集
     * @param array $keys   需要获取的键名集合
     * @param bool $is_index_value   是否需要重建为索引数组 true or false
     * @return array
     */
    public static function intersectKeyFlip(array $array, array $keys, $is_index_value = false) : array
    {
        $res = array_intersect_key($array, array_flip($keys));
        return $is_index_value ? array_values($res) : $res;
    }
