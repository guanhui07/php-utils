...
use Raylin666\Contract\JsonableInterface;
use Traversable;
 ...
 */
class CollectionHelper implements ArrayAccess, ArrayableInterface, Countable, IteratorAggregate, JsonableInterface, JsonSerializable
{
    /**
    ...
    public function containsStrict($key, $value = null): bool
    {
        if (func_num_args() === 2) {
            return $this->contains(function ($item) use ($key, $value) {
                return ArrayHelper::get($item, $key) === $value;
            });
        }
    ...
    {
        return new static(ArrayHelper::crossJoin(
    ...
    protected function operatorForWhere($key, $operator = null, $value = null): callable
    {
        ...
        return function ($item) use ($key, $operator, $value) {
            $retrieved = ArrayHelper::get($item, $key);
    ...
    public function whereIn($key, $values, $strict = false)
    {
        ...
        return $this->filter(function ($item) use ($key, $values, $strict) {
            return in_array(ArrayHelper::get($item, $key), $values, $strict);
        });
    }
    ...
    public function groupBy($groupBy, $preserveKeys = false)
    {
        ...
        if (!empty($nextGroups)) {
            return $result->map(function (CollectionHelper $value) use ($nextGroups, $preserveKeys) {
     ...
     *
     * e.g. new CollectionHelper([1, 2, 3])->zip([4, 5, 6]);
     *      => [[1, 4], [2, 5], [3, 6]]
     *
     * @param mixed ...$items
     *
     ...
     */
    public function toBase(): CollectionHelper
    {
    ...
    /**
     * Results array of items from CollectionHelper or ArrayableInterface.
     *
     * @param mixed $items
     *
     * @return array
    ...
    protected function getArrayableItems($items): array
    {
        ...
        } else if ($items instanceof JsonableInterface) {
